  ...
  return (
    <S.SchedLi>
      <S.Lab>
        ...
        {lab === "hanul" && (
          <S.RenterNum>
            총 {renterList.reduce((a, c) => a + c.rentalAmount, 0)}명 대여
          </S.RenterNum>
        )}
        {acceptTime && receive ? (
          ...
          <Button
            className={
              dayjs().format("YYYY-MM-DD") === selectDate ? "main" : "gray"
            }
      ...
      <ul>
        {renterList.map((renterItem) => (
          <S.RenterLi
            className={renterList.length === 1 ? "only" : ""}
            key={renterItem.id}
          >
            <S.PurposeBtn
              onClick={(e) => handleGetPurpose(e, renterItem.reservationId)}
            >
              <img src={iconMsg} alt="" />
            </S.PurposeBtn>
            <p>
              {renterItem.renterName} &emsp; {renterItem.memberNumber}
            </p>
            ...
            {receive && !acceptTime && (
              <Button
                ...
                className="sub"
                fontSize="14px"
                onClick={() => handleCancelModal(renterItem)}
              />
            )}
          </S.RenterLi>
        ))}
