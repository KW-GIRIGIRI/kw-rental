...
export default function ChangeAccount({ setCheckPw }) {
  ...
  return (
    isAuth ?
      <>
        <S.AuthTit>공동 관리자와 비밀번호를 <ins>반드시</ins> 공유하세요.</S.AuthTit>
        <Form onSubmit={handleSubmit(handleAuthChangeAccount)}>
          <label>비밀번호</label>
          <InpWrap>
            <Input type={showPw.password ? 'password' : 'text'}
              {...register('password', {
                required: {
                  value: true,
                  message: '비밀번호를 입력하세요.'
            ...
            />
            <PwImg bottom={showPw.password ? '22px' : '24px'} onClick={() => setShowPw({...showPw, password : !showPw.password})} src={showPw.password ? iconBlockPw : iconShowPw} alt="" />
          </InpWrap>
          {errors.password && <ErrText>{errors.password.message}</ErrText>}

          <label>비밀번호 확인</label>
          <InpWrap>
            <Input type={showPw.passwordCheck ? 'password' : 'text'}
            ...
            />
            <PwImg bottom={showPw.passwordCheck ? '22px' : '24px'} onClick={() => setShowPw({...showPw, passwordCheck : !showPw.passwordCheck})} src={showPw.passwordCheck ? iconBlockPw : iconShowPw} alt="" />
          </InpWrap>
          {errors.passwordConfirm && <ErrText>{errors.passwordConfirm.message}</ErrText>}

          <Button text='설정하기' className='main' padding='12px 33px' borderRadius='10px' fontSize='14px' margin='29px 0' />
        </Form>
      </>
      :
        <>
        ...
        <Form onSubmit={handleSubmit(handleChangeAccount)}>
          ...
          <InpWrap className="email">
            <Input type='text' placeholder="gildong1234"
              {...register('emailF', {
                required: {
                  value: true,
                  message: '이메일을 입력하세요.'
          ...
          {(errors.emailF || errors.emailS)  && <ErrText>{errors.emailF?.message || errors.emailS?.message }</ErrText>}
