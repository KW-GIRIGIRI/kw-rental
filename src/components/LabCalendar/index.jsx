...
const LabCalendar = forwardRef((props, ref) => {
  ...
  const handleGetCalendarLabRemain = async () => {
    ...
    if (hanul) {
      ...
      res.remainQuantities.length &&
        res.remainQuantities.forEach((item) => {
      ...
      res.remainReservationCounts.length &&
        res.remainReservationCounts.forEach((item) => {
          newObj[~~item.date.split("-").slice(-1)[0]] =
            item.remainReservationCount;
        });
  ...
  const handleSetDate = (e) => {
    ...
    if (isAuth) {
      ...
      if (
        ...
        ~~selectDate.format("YYMMDD") <
          ~~dayjs().add(1, "month").format("YYMMDD")
      ) {
        dispatch(
          setLabDate(
            dayjs(`${thisYear}-${thisMonth + 1}-${e.target.value}`).format(
  ...
  const handleCalendar = (i) => {
    if (!isAuth) {
      if (
        dayjs(`${dayObj.year()}-${dayObj.month() + 1}-${i + 1}`).valueOf() >
          dayjs().valueOf() &&
        dayjs(`${dayObj.year()}-${dayObj.month() + 1}-${i + 1}`).valueOf() <
    ...
    <S.Wrapper>
      <S.Header>
        ...
        <S.NavBtn
          onClick={handleNext}
          disabled={
            !isAuth && dayjs().add(1, "month").month() === dayObj.month()
          }
        >
          ...
          .map((_, i) => (
            ...
            >
              ...
              {Object.keys(seatArray).length ? (
                dayjs(
                  `${dayObj.year()}-${dayObj.month() + 1}-${i + 1}`
                ).day() <= operationDay[operationDay.length - 1] &&
                dayjs(
                  `${dayObj.year()}-${dayObj.month() + 1}-${i + 1}`
                ).day() >= operationDay[0] &&
                handleCalendar(i) &&
                (dayjs(
                  `${dayObj.year()}-${dayObj.month() + 1}-${i + 1}`
                ).format("YYMMDD") === dayjs(selectDate).format("YYMMDD") ? (
                  <ins>
                    {hanul
                      ? `대여(${seatArray[i + 1] || 0}/16)`
                      : seatArray[i + 1] > 0
                      ? "대여 가능"
                      : "대여 불가"}
                  </ins>
